function exitcode = preproc_min1pipe(file_name, path_name, MIN1PIPE_dir)
    addpath(genpath(MIN1PIPE_dir)) % add MIN1PIPE repo to matlab path, this will prevent the proper installation of cvx toolbox in a fresh machine.

    %% session-specific parameter initialization %% 
    Fsi = 25;
    Fsi_new = 25; %%% no temporal downsampling %%%
